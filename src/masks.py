...
def get_mask_card_number(card_number: str) -> str:
    ...
    if not card_number.isdigit() or len(card_number) != 16:
        ...
        logger.error(
            f"Провал: Некорректный номер карты '{card_number}'. Ожидалось 16 цифр."
        )
...
def get_mask_account(account_number: str) -> str:
    ...
    """
    logger.debug(
        f"Старт маскирования счета. Входные данные: ********{account_number[-4:]}"
    )

    if not account_number.isdigit() or len(account_number) < 4:
        # Логируем ошибку, если номер счета некорректен
        logger.error(
            f"Провал: Некорректный номер счета '{account_number}'. Ожидалось минимум 4 цифры."
        )
